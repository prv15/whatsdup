...
export function MetaConnectionPage() {
  const queryClient = useQueryClient(); const [sdkReady, setSdkReady] = useState(false); const [signupError, setSignupError] = useState(''); const [signupSuccess, setSignupSuccess] = useState('');
  ...
  const complete = useMutation({ mutationFn: async (payload: { code: string; wabaId: string; phoneNumberId: string; reconnect: boolean }) => (await api.post<{ data: MetaConnectionStatus }>(payload.reconnect ? '/meta/connection/reconnect' : '/meta/connection/complete', payload)).data.data, onSuccess: (data, variables) => { queryClient.setQueryData(['meta-connection'], data); setSignupSuccess(variables.reconnect ? 'Meta authorization refreshed successfully. Your WhatsApp assets and existing data were preserved.' : 'Meta connection completed successfully.'); }, onError: (error) => setSignupError(apiError(error)) });
  const verifyBusinessAccess = useMutation({ mutationFn: async () => (await api.post<{ data: { verified: boolean; businessCount: number; message: string } }>('/meta/connection/verify-business-access')).data.data, onSuccess: (data) => { setSignupError(''); setSignupSuccess(data.message); queryClient.invalidateQueries({ queryKey: ['meta-connection'] }); }, onError: (error) => { setSignupSuccess(''); setSignupError(apiError(error)); } });

  const state = connection.data; const reconnecting = state?.status === 'connected' || state?.status === 'webhook_error';
  const tryComplete = () => { if (codeRef.current && assetsRef.current && !complete.isPending) complete.mutate({ code: codeRef.current, ...assetsRef.current, reconnect: reconnecting }); };
  useEffect(() => {
  ...
  const startSignup = () => {
    const config = configuration.data; setSignupError(''); setSignupSuccess(''); codeRef.current = null; assetsRef.current = null;
  ...
  const configured = configuration.data?.enabled === true; const canConnect = configured && sdkReady && !configuration.data?.requiresHttps && (state?.status === 'not_connected' || reconnecting);
  return <div className="space-y-8"><div><p className="text-sm font-semibold text-brand-700">Official Cloud API</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Meta Connection</h1><p className="mt-2 max-w-3xl text-muted">Connect your Meta business portfolio, WhatsApp Business Account and phone number through Meta’s official Embedded Signup.</p></div>
    ...
    {configuration.data?.testConnection && !configuration.data.requiresHttps && <TestConnectionForm config={configuration.data.testConnection} onConnected={(data) => { queryClient.setQueryData(['meta-connection'], data); queryClient.invalidateQueries({ queryKey: ['templates'] }); setSignupError(''); setSignupSuccess(''); }} />}
  </div>;
}
