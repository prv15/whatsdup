...
export function LoginPage() {
  ...
  const existingUser = authStore.getSnapshot().user;
  if (existingUser) return <Navigate to={existingUser.scope === 'platform' ? '/admin' : '/dashboard'} replace />;
  const submit = handleSubmit(async (values) => { setServerError(''); try { const { data } = await api.post<AuthPayload>('/auth/login', values); authStore.setSession(data.accessToken, data.user); const requested = (location.state as { from?: string } | null)?.from; navigate(requested ?? (data.user.scope === 'platform' ? '/admin' : '/dashboard'), { replace: true }); } catch { setServerError('We could not sign you in. Check your details or try again shortly.'); } });
