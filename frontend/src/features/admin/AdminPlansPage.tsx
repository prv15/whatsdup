...
const schema = z.object({
  ...
  priceRupees: z.string().trim().refine((value) => value === '' || (/^\d+(\.\d{1,2})?$/.test(value) && Number(value) <= 1_000_000), 'Enter a valid price'),
...
const emptyValues: Values = { name: '', code: '', description: '', priceRupees: '', billingInterval: 'month', status: 'active', isPublic: true, sortOrder: 0, phoneNumbers: '', teamMembers: '', contacts: '', monthlyRecipients: '', features: '' };
...
export function AdminPlansPage() {
  ...
  const openEdit = (plan: AdminPlan) => { reset({ name: plan.name, code: plan.code, description: plan.description, priceRupees: plan.priceMinor === null ? '' : String(plan.priceMinor / 100), billingInterval: plan.billingInterval, status: plan.status, isPublic: plan.isPublic, sortOrder: plan.sortOrder, phoneNumbers: displayLimit(plan.limits.phoneNumbers), teamMembers: displayLimit(plan.limits.teamMembers), contacts: displayLimit(plan.limits.contacts), monthlyRecipients: displayLimit(plan.limits.monthlyRecipients), features: plan.features.join('\n') }); setEditing(plan); };
  ...
  return <div className="space-y-8">
    ...
    {editing && <section aria-label={editing === 'new' ? 'Create plan' : 'Edit plan'} className="rounded-2xl border border-line bg-white p-6 shadow-card"><div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-semibold">{editing === 'new' ? 'Create subscription plan' : `Edit ${editing.name}`}</h2><p className="mt-1 text-sm text-muted">Blank limits mean custom or unlimited by agreement.</p></div><button aria-label="Close form" onClick={closeEditor} className="rounded-lg p-2 hover:bg-gray-100"><X/></button></div>
      <form className="mt-6 grid gap-5 md:grid-cols-2" onSubmit={handleSubmit((values) => save.mutate(values))}>
        ...
        <Field label="Price in rupees" error={errors.priceRupees?.message}><input inputMode="decimal" className="input" placeholder="Leave blank for custom" {...register('priceRupees')}/></Field><Field label="Billing interval" error={errors.billingInterval?.message}><select className="input" {...register('billingInterval')}><option value="month">Monthly</option><option value="year">Yearly</option><option value="custom">Custom</option></select></Field>
...
function toPayload(values: Values) { return { name: values.name, code: values.code, description: values.description, priceMinor: values.priceRupees === '' ? null : Math.round(Number(values.priceRupees) * 100), currency: 'INR', billingInterval: values.billingInterval, status: values.status, isPublic: values.isPublic, sortOrder: values.sortOrder, limits: { phoneNumbers: toLimit(values.phoneNumbers), teamMembers: toLimit(values.teamMembers), contacts: toLimit(values.contacts), monthlyRecipients: toLimit(values.monthlyRecipients) }, features: values.features.split('\n').map((feature) => feature.trim()).filter(Boolean) }; }
function formatPrice(plan: AdminPlan) { return plan.priceMinor === null ? 'Custom' : new Intl.NumberFormat('en-IN', { style: 'currency', currency: plan.currency, maximumFractionDigits: 0 }).format(plan.priceMinor / 100); }
