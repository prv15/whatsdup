...
export function App() {
  ...
  return <Suspense fallback={<div className="grid min-h-screen place-items-center">Loading…</div>}><Routes><Route path="/login" element={<LoginPage/>}/><Route path="/forgot-password" element={<ForgotPasswordPage/>}/><Route path="/reset-password" element={<ResetPasswordPage/>}/><Route element={<BusinessProtected/>}><Route path="/dashboard" element={<DashboardPage/>}/><Route path="/meta" element={<MetaConnectionPage/>}/><Route path="/contacts" element={<ContactsPage/>}/><Route path="/templates" element={<TemplatesPage/>}/><Route path="/campaigns" element={<CampaignsPage/>}/>{['reports', 'settings'].map((route) => <Route key={route} path={`/${route}`} element={<MilestoneNoticePage title={route[0]!.toUpperCase() + route.slice(1)}/>}/>)}</Route><Route element={<PlatformProtected/>}><Route path="/admin" element={<AdminDashboardPage/>}/><Route path="/admin/businesses" element={<AdminBusinessesPage/>}/><Route path="/admin/users" element={<AdminUsersPage/>}/><Route path="/admin/plans" element={<AdminPlansPage/>}/></Route><Route path="*" element={<HomeRedirect/>}/></Routes></Suspense>;
}
